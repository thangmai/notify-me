(ns pallet.crate.notify-me
  "A pallet crate to install and configure notify me service"
  (:require
...
(def ^{:doc "Flag for recognising changes to configuration"}
  notify-me-config-changed-flag "notify-me-config")

;;; # Settings
(defn service-name
  "Return a service name for notify-me."
  [{:keys [instance-id] :as options}]
  (str "notify-me" (when instance-id (str "-" instance-id))))
...
(defn default-settings [options]
  {:version "0.4.0"
   ...
   :group "notifier"
   :home "/opt/notify-me"
   :config-dir (fragment (file (config-root) "notify-me"))
   :log-dir (fragment (file (log-root) "notify-me"))
   :supervisor :runit
...
(defn run-command
  "Return a script command to run notify-me."
  [{:keys [home user config-dir] :as settings}]
  (fragment ((file ~home "bin" "notify-me") (file ~config-dir "notify-me.conf"))))
...
(defplan settings
  ...
  (let [settings (merge (default-settings options) settings)
    ...
    (assoc-settings :notify-me settings {:instance-id instance-id})
    (supervisor-config :notify-me settings (or options {}))))

;;; # User
(defplan user
  "Create the notify-me user"
  [{:keys [instance-id] :as options}]
  (let [{:keys [user owner group home]} (get-settings :notify-me options)]
...
(defplan install
  ...
  (let [{:keys [owner group log-dir] :as settings}
        (get-settings :notify-me {:instance-id instance-id})]
    (crate-install/install :notify-me instance-id)
    (when log-dir
...
(defplan config-file
  ...
  (apply-map
   ...
   :flag-on-changed notify-me-config-changed-flag
...
(defplan configure
  ...
  [{:keys [instance-id] :as options}]
  (let [{:keys [config] :as settings} (get-settings :notify-me options)]
    (debugf "configure %s %s" settings options)
    (config-file settings "notify-me.conf" {:content (str config)})))

;;; # Run
(defplan service
  "Run the notify-me service."
...
(defn server-spec
  "Returns a server-spec that installs and configures notify-me."
  [settings & {:keys [instance-id] :as options}]
  (api/server-spec
   :phases
   {:settings (plan-fn (pallet.crate.notify-me/settings (merge settings options)))
    :install (plan-fn
               (user options)
               (install options))
    :configure (plan-fn
